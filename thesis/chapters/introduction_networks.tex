\section{Networks in biology}
Instead of considering each gene as independent and static, the analyses reported in this chapter aim at recovering the holistic and dynamic structure of the transcriptome.

\subsection{Overview: network design and paradigms}

\paragraph{Top-down vs. Bottom-up}
\paragraph{Knowledge vs. Pure Learning approaches}
\todo[fancyline]{refer to the Boolean network internship}
\paragraph{Model graphs}

\begin{enumerate}[label=\alph*)]
\item[Boolean networks:]
\item[Random networks:]
\item[Mechanistic networks:]

\end{enumerate}

\subsection{\glsfmtfull{ggm}}
\subsubsection{Conditional independence in multivariate Gaussian distributions}
\subsubsection{Learn \glsfmtlong{ggm} in high-dimensional setting}
\subsubsection{From \glsfmtlong{ggm} to \glsfmtlong{gbn}}

\paragraph{Overview of \glsfmtlong{gbn}}
\paragraph{Convex optimisation under constraint}
\paragraph{Learn a \gls{gbn} under topological constraints}

\todo[fancyline]{Dahl, 2005, Maximum likelihood estimation of Gaussian graphical models: Numerical implementation and topology selection}
\subsubsection{Differential network analysis}

\subsection{Dynamic identification of pathways}

% Predict dynamically the behaviour of a new drug and prioritise biomarkers integrating the graph structure